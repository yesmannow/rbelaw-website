/**
 * Firecrawl Attorney Scraper
 *
 * Extracts structured attorney data from rbelaw.com using Firecrawl v2 scrape with extract.
 *
 * Run with: tsx scripts/firecrawl-scrape-attorneys.ts --in=data/site-map.json --out=data/attorneys.json
 *
 * Options:
...
import { normalizeAttorneyExtract } from './lib/normalize.js'
...
const ATTORNEY_SCHEMA = {
  ...
  properties: {
    ...
    representativeMatters: { type: 'array', items: { type: 'string' } },
    awards: { type: 'array', items: { type: 'string' } },
    publications: { type: 'array', items: { type: 'string' } },
  },
  required: ['name', 'bioMarkdown'],
}

// Extraction prompt
const EXTRACTION_PROMPT = `
Extract attorney information from this page. Focus on:
- Name and title
- Contact information (email, phone, LinkedIn)
- Headshot image URL
- Biography text (convert to markdown)
...
async function processAttorneys(attorneyUrls: string[], concurrency: number, force: boolean, resume: boolean, retryFailed: boolean) {
  ...
  while (results.length < attorneyUrls.length) {
...
async function main() {
  ...
  console.log('🚀 Starting attorney scraping\n')
  ...
  try {
    ...
    if (!siteMap.attorneys || !Array.isArray(siteMap.attorneys)) {
      throw new Error('No attorneys array found in site map')
    }

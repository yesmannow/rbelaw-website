...
- [ ] No Vite artifacts exist:
  ...
  - [ ] `vite` is NOT in dependencies/devDependencies
  - [ ] `react-router-dom` is NOT in dependencies/devDependencies
- [ ] PWA manifest is Next.js-native:
  - [ ] `app/manifest.ts` (or `src/app/manifest.ts`) exists
  - [ ] No static `public/manifest.json` required (unless deliberately used and verified)
  - [ ] referenced icons exist under `/public/icons/...`
- [ ] Payload + DB deploy invariants:
  - [ ] Vercel build command runs migrations before build:
    - [ ] `payload migrate && next build`
  - [ ] `DATABASE_URL` (pooled) present for runtime
  - [ ] `DIRECT_DATABASE_URL` (direct) present for migrations/DDL
  - [ ] DB schema exists before SSG (tables like `attorneys`, `practice_areas`)
...
## Fresh DB Deploy Safe-Mode Strategy

When deploying to a fresh database (or when the database is temporarily unreachable), the build can fail because `generateStaticParams()` tries to query tables that don't exist yet (Postgres `42P01`). This section describes two deployment modes to handle this scenario.

### Mode A: Normal Deployment (DB Exists)
...
### Mode B: Fresh DB / Bootstrap Deployment

**When to use**: First-time deployments, database migrations, or when DB access might be temporarily blocked.
...
#### Guardrail 1: Fail-Soft `generateStaticParams()`

In routes like `app/attorneys/[slug]/page.tsx` and `app/practice-areas/[slug]/page.tsx`, use this pattern:
...
**Note**: Your existing routes already implement this pattern! ✅

#### Guardrail 2: Environment Flag for Safe-Mode

Add `FRESH_DB_SAFE_MODE=1` in Vercel for bootstrap deploys.

Then in `generateStaticParams()`:
...
#### Guardrail 3: Runtime Fallback Rendering

Ensure pages can render even without prebuilt params:

- If content is missing, show a "Content coming soon" state
- If `notFound()`, be intentional (don't throw during build)
...
export default async function AttorneyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const payload = await getPayload({ config })
    const attorney = await payload.find({
      collection: 'attorneys',
      where: { slug: { equals: slug } },
      limit: 1,
    })

    if (!attorney.docs.length) {
      notFound() // Intentional 404, not a build error
    }

    return <AttorneyContent attorney={attorney.docs[0]} />
  } catch (error) {
    // Runtime fallback: show placeholder or error state
...
**Note**: Use bootstrap mode only for initial deployments. Once the database is set up, switch back to normal mode for stricter validation.
